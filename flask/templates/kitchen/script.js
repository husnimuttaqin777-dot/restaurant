...
function generateOrder({
    table,
    itemsData, // [{ itemIndex: 0, quantity: 2, note: "Less spicy" }]
    server = null,
    guests = 1,
    status = 'new'
}) {
    const items = itemsData.map(data => {
        const item = menuItems[data.itemIndex];
    ...
    return {
        ...
        readyAt: null,
        completedAt: null,
        server: server || serverNames[Math.floor(Math.random() * serverNames.length)],
...
*/
// --- Simulate New Order ---
function simulateNewOrder(data1) {
	
	console.log("Siapa yang manggil?", simulateNewOrder.caller);
    console.log("Tipe data data1:", typeof data1);
    console.log("Nilai data1:", data1);
